"""Train a DG-DANN baseline that removes source-domain information only."""
...
def main() -> None:
    ...
    metrics_csv = results_dir / "metrics" / "dg_dann_metrics.csv"
    checkpoint_path = results_dir / "checkpoints" / "dg_dann_model.pt"
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, y, d in train_loader:
            x = x.to(device)
            y = y.to(device)
            d_binary = d.to(device)

            class_logits, domain_logits, _ = model(x, lambda_grl=lambda_adv)
            loss_cls = criterion_cls(class_logits, y)
            loss_domain = criterion_domain(domain_logits, d_binary)
        ...
        unseen_target_acc, _, _ = evaluate_classifier(
            model,
            test_loader,
            device,
            predict_fn=lambda m, x: m.predict(x),
