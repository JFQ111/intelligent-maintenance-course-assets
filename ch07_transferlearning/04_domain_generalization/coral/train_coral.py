"""Train a CORAL-based DG baseline on JNU source domains 600 and 800."""
...
def main() -> None:
    ...
    metrics_csv = results_dir / "metrics" / "coral_metrics.csv"
    checkpoint_path = results_dir / "checkpoints" / "coral_model.pt"
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, y, d in train_loader:
            ...
            domain_values = torch.unique(d)
            loss_coral_value = torch.zeros((), device=device)
            if len(domain_values) >= 2:
                source_a = features[d == domain_values[0]]
                source_b = features[d == domain_values[1]]
                loss_coral_value = coral_loss(source_a, source_b)

            total_loss = loss_cls + lambda_coral * loss_coral_value
            ...
            running_coral += float(loss_coral_value.item())
        ...
        row = {
            ...
            "loss_coral": running_coral / max(1, num_batches),
        ...
        print(
            f"[DG-CORAL] epoch={epoch:02d} "
            f"loss_cls={row['loss_cls']:.4f} "
            f"loss_coral={row['loss_coral']:.4f} "
